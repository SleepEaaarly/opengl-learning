...
void renderQuad();
void renderCube();
GLfloat lerp(GLfloat a, GLfloat b, GLfloat w);
...
int main()
{
    ...
    // -------------------------
    Shader shaderGeo("shaders/ssao_geometry.vs", "shaders/ssao_geometry.fs");
    Shader shaderSSAO("shaders/ssao.vs", "shaders/ssao.fs");
    Shader shaderLight("shaders/ssao.vs", "shaders/ssao_lighting.fs");
    Shader shaderBlur("shaders/ssao.vs", "shaders/ssao_blur.fs");

    // Set samplers
    shaderLight.use();
    shaderLight.setInt("gPositionDepth", 0);
    ...
    // Model cyborg("model/mary/Marry.obj");
    ...
    // tell opengl color attachment to render
    ...
    for (int i = 0; i < 16; i++) {
        glm::vec3 noise(
            2.0 * randomFloats(generator) - 1.0,
            2.0 * randomFloats(generator) - 1.0,
            0.0
        );
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glm::mat4 model;
        shaderGeo.use();
        shaderGeo.setMat4("projection", projection);
        shaderGeo.setMat4("view", view);

        // Floor cube
        model = glm::translate(model, glm::vec3(0.0, -1.0f, 0.0f));
        model = glm::scale(model, glm::vec3(20.0f, 1.0f, 20.0f));
        shaderGeo.setMat4("model", model);
        renderCube();
        ...
        for (GLuint i = 0; i < 64; ++i)
            shaderSSAO.setVec3("samples[" + std::to_string(i) + "]", ssaoKernal[i]);
