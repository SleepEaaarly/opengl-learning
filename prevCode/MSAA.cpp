...
int main()
{
    ...
    Shader planetShader("shaders/10.3.planet.vs", "shader/10.3.planet.fs");

    // load models
    Model rock("model/rock/rock.obj");
    Model planet("model/planet/planet.obj");

    // generate model transformation matrices
    ...
    for (int i = 0; i < amount; i++) {
        ...
        // 1.translation
        float angle = (float) i / amount * 2 * PI;
        float displacement = (rand() % (int)(2 * offset * 100)) / 100.f - offset;
        float x = radius * sin(angle) + displacement;
        displacement = (rand() % (int)(2 * offset * 100)) / 100.f - offset;
        float z = radius * cos(angle) + displacement;
        displacement = (rand() % (int)(2 * offset * 100)) / 100.f - offset;
        float y = 0.4 * displacement;
        model = glm::translate(model, glm::vec3(x, y, z));
        
        // 2.rotation
        float rotAngle = static_cast<float>(rand() % 360) / 360.f * 2 * PI;
